...
- [x] **Layout Template**: Xây dựng `AdminLayout.tsx` cho toàn bộ các trang Admin:
  - **Sidebar (Menu điều hướng)**:
    - Sử dụng Component `Accordion` của shadcn/ui để tạo cấp menu cha - con.
    - Trạng thái các menu cha mặc định mở rộng toàn bộ (`defaultValue`).
    - Nút Toggle thu gọn/mở rộng Sidebar có lưu trạng thái vào `localStorage` (tránh bị reset khi f5).
    - Áp dụng CSS `flex-1 overflow-y-auto scrollbar-hide` chỉ cho thẻ `<nav>` để cố định Logo và User Info ở 2 đầu.
  - **Header (Thanh công cụ trên)**:
    - Hiển thị thông tin Admin (Avatar, Email, Trạng thái online) qua `DropdownMenu`.
    - Gọi API lấy User Data từ `AuthContext` để đồng bộ State toàn hệ thống.
  - **Quản lý Thông tin cá nhân (`PersonalInfoModal.tsx`)**:
    - Chuẩn hóa form bằng nguyên tắc thiết kế Mobile-first và UI Error trong `MODAL-UI-RULES.md`.
    - Tích hợp tính năng Upload Ảnh Đại Diện (`handleFileUpload`) trực tiếp qua API `POST /upload/`.
    - Gọi API `PATCH /auth/me/` để cập nhật (Họ, Tên, Số điện thoại) và `fetchUser` để làm mới Header.
    - Tự động bắt lỗi Input (Border đỏ + Inline error message) nếu bỏ trống các trường bắt buộc, tự động reset lỗi khi nhập tiếp hoặc mở lại form.
  - **Đổi mật khẩu (`ChangePasswordModal.tsx`)**:
    - Xác thực kỹ lưỡng Form Data (Mật khẩu mới >= 8 ký tự, 2 trường mật khẩu mới phải trùng khớp hoàn toàn).
    - Gọi API `POST /auth/password/change/` để cập nhật bảo mật qua Token.
    - Xử lý mượt mà trạng thái vô hiệu hóa nút (Disabled) và hiển thị icon loading (`Loader2`) trong lúc call API.
...
- [x] **Lịch sử giao dịch (`/admin/transactions`)**:
...
- [x] **1. Nạp tiền thủ công (`/admin/deposits/manual`)**:
...
- [x] **2. Quản lý Thẻ nạp (`/admin/deposits/cards`)**:
...
- [x] **Tin tức (`/admin/news`)**: Quản lý bài viết, chuyên mục, SEO. Tích hợp trình soạn thảo Rich Text (VD: TipTap / CKEditor).
